...
final class ScreenStreamer {
    ...
    func start() {
        ...
        guard CGPreflightScreenCaptureAccess() else {
            Diagnostics.geometry.notice("stream skipped: screen recording permission is not granted")
            return
        }
    ...
    private func begin(displayID: CGDirectDisplayID, on target: NSScreen) async {
        ...
        do {
            ...
            try fresh.addStreamOutput(
                receiver,
                type: .screen,
                sampleHandlerQueue: DispatchQueue(label: "DuoFold.frames", qos: .userInteractive)
            )
